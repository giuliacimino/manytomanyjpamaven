...
public class RuoloServiceImpl implements RuoloService {
	...
	public void rimuovi(Long idRuoloToRemove) throws Exception {
		...
		try {
			...
			if (utenteDAO.findAllByRuolo(ruoloDAO.get(idRuoloToRemove)).size() > 0) {
				throw new RuoloConUtenteAssociatoException("problema giulia in input");
			}
	...
	@Override
	public List<String> cercaDistinctDescrizioniRuoloConUtente() throws Exception {
		EntityManager entityManager = EntityManagerUtil.getEntityManager();
		...
		try {
			...
			// eseguo quello che realmente devo fare
			return ruoloDAO.findDistinctDescrizioneRuoloWithUtenti();
